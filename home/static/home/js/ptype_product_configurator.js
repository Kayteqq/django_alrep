...
const LONG_ARC = 2;
const SHORT_ARC = 1;
const BIG_STROKE_WIDTH = 0.4;
const SMALL_STROKE_WIDTH = 0.2;
const MEASURING_LINE_LENGTH = 10;
const MEASUREMENT_MARGIN = 8.5;
        ...
        schemeLAngle(widthG, widthE, widthH, angleAlfa)
        {
            ...
            return `
                <svg class="configurated"  
                    ...
                    <line x1="${SHAPE_START_X}" x2="${SHAPE_START_X}" y1="${SHAPE_START_Y}" y2="${SHAPE_START_Y + widthH + MEASURING_LINE_LENGTH}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
                    <line x1="${SHAPE_START_X + widthE}" x2="${SHAPE_START_X + widthE}" y1="${SHAPE_START_Y}" y2="${SHAPE_START_Y + widthH + MEASURING_LINE_LENGTH}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
                    <line x1="${SHAPE_START_X + widthE - (Math.cos(alfaRadians) * widthG)}" x2="${SHAPE_START_X + widthE + (Math.cos(alfaRadians) * MEASURING_LINE_LENGTH)}" y1="${SHAPE_START_Y + (Math.sin(alfaRadians) * widthG)}" y2="${SHAPE_START_Y - (Math.sin(alfaRadians) * MEASURING_LINE_LENGTH)}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
                    <line x1="${SHAPE_START_X + widthE + (Math.sin(alfaRadians) * widthH)}" x2="${SHAPE_START_X + widthE + (Math.cos(alfaRadians) * MEASURING_LINE_LENGTH) + (Math.sin(alfaRadians) * widthH)}" y1="${SHAPE_START_Y + (Math.cos(alfaRadians) * widthH)}" y2="${SHAPE_START_Y + (Math.cos(alfaRadians) * widthH) - (Math.sin(alfaRadians) * MEASURING_LINE_LENGTH)}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>

                    <line x1="${SHAPE_START_X + widthE + (Math.cos(alfaRadians)*MEASUREMENT_MARGIN)}" x2="${SHAPE_START_X + widthE + (Math.sin(alfaRadians) * widthH) + (Math.cos(alfaRadians)*MEASUREMENT_MARGIN)}" y1="${SHAPE_START_Y-(Math.sin(alfaRadians)*MEASUREMENT_MARGIN)}" y2="${SHAPE_START_Y + (Math.cos(alfaRadians) * widthH) - (Math.sin(alfaRadians)*MEASUREMENT_MARGIN)}" stroke="gray" stroke-width="${BIG_STROKE_WIDTH}" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)"/>
                    <text x="${SHAPE_START_X + widthE + (0.5 * widthH) - 4}" y="${SHAPE_START_Y - MEASUREMENT_MARGIN - 2.5}" font-size="10" font-family="sans-serif" transform="rotate(${180-angleAlfa} ${SHAPE_START_X + widthE} ${SHAPE_START_Y})" fill="gray">H</text> 

                    <line x1="${SHAPE_START_X}" x2="${SHAPE_START_X + widthE}" y1="${SHAPE_START_Y + widthH + MEASUREMENT_MARGIN}" y2="${SHAPE_START_Y + widthH + MEASUREMENT_MARGIN}" stroke="gray" stroke-width="${BIG_STROKE_WIDTH}" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)"/>
                    <text x="${SHAPE_START_X + (0.5 * widthE) - 2}" y="${SHAPE_START_Y + widthH + MEASUREMENT_MARGIN - 2.5}" font-size="10" font-family="sans-serif" fill="gray">E</text> 

                    <line x1="${SHAPE_START_X + positionGLabel}" x2="${SHAPE_START_X + positionGLabel}" y1="${SHAPE_START_Y}" y2="${SHAPE_START_Y-(0.5*MEASURING_LINE_LENGTH)}" stroke="gray" stroke-width="${BIG_STROKE_WIDTH}" marker-start="url(#arrowhead)"/>
                    <line x1="${SHAPE_START_X + positionGLabel}" x2="${SHAPE_START_X + positionGLabel}" y1="${SHAPE_START_Y+widthG}" y2="${SHAPE_START_Y+widthG+(0.5*MEASURING_LINE_LENGTH)}" stroke="gray" stroke-width="${BIG_STROKE_WIDTH}" marker-start="url(#arrowhead)"/>
                    <text x="${SHAPE_START_X + positionGLabel + (0.5 * MEASURING_LINE_LENGTH)}" y="${SHAPE_START_Y}" font-size="10" font-family="sans-serif" transform="rotate(-90 ${SHAPE_START_X + positionGLabel} ${SHAPE_START_Y})" fill="gray">g</text> 
                

                    <path d="
                        M ${SHAPE_START_X} ${SHAPE_START_Y}
                        L ${SHAPE_START_X + widthE} ${SHAPE_START_Y}
                        ...
                        L ${SHAPE_START_X + widthE - Math.cos(alfaRadians) * widthG} ${SHAPE_START_Y + widthG}
...
<!--                                        E - line -->
                    <line x1="${SHAPE_START_X + widthA * Math.sin(betaRadians)}" x2="${SHAPE_START_X + widthA * Math.sin(betaRadians)}" y1="${SHAPE_START_Y + widthA * Math.cos(betaRadians)}" y2="${SHAPE_START_Y + widthH + widthA * Math.cos(betaRadians) + MEASURING_LINE_LENGTH}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
                    <line x1="${SHAPE_START_X + widthA * Math.sin(betaRadians) + widthE}" x2="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)}" y1="${SHAPE_START_Y + widthA * Math.cos(betaRadians) - widthG}" y2="${SHAPE_START_Y + widthH + widthA * Math.cos(betaRadians) + MEASURING_LINE_LENGTH}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
<!--                                        E - text-->
                    <line x1="${SHAPE_START_X + widthA * Math.sin(betaRadians)}" x2="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)}" y1="${SHAPE_START_Y + widthH  + MEASUREMENT_MARGIN + widthA * Math.cos(betaRadians)}" y2="${SHAPE_START_Y + widthH + MEASUREMENT_MARGIN + widthA * Math.cos(betaRadians)}" stroke="gray" stroke-width="${BIG_STROKE_WIDTH}" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)"/>
                    <text x="${SHAPE_START_X + (0.5 * widthE) - 2 + widthA * Math.sin(betaRadians)}" y="${SHAPE_START_Y + widthH  + MEASUREMENT_MARGIN + widthA * Math.cos(betaRadians)- 2.5}" fill="gray" font-size="10" font-family="sans-serif">E</text> 

<!--                                        H - line -->
                    <line x1="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)}" x2="${SHAPE_START_X + widthE + MEASURING_LINE_LENGTH * Math.cos(alfaRadians) + widthA * Math.sin(betaRadians)}" y1="${SHAPE_START_Y + widthA * Math.cos(betaRadians) - widthG}" y2="${SHAPE_START_Y + widthA * Math.cos(betaRadians) - widthG - MEASURING_LINE_LENGTH * Math.sin(alfaRadians) }" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
...
<!--                                        G - lext -->
                    ...
                    <text x="${SHAPE_START_X + positionGLabel + widthA * Math.sin(betaRadians) + 0.5 * MEASURING_LINE_LENGTH}" y="${SHAPE_START_Y + widthA * Math.cos(betaRadians) - widthG}" font-size="10" font-family="sans-serif" transform="rotate(-90 ${SHAPE_START_X + positionGLabel + widthA * Math.sin(betaRadians)} ${SHAPE_START_Y + widthA * Math.cos(betaRadians) - widthG})" fill="gray">g</text>
        ...
        schemeCAngle(widthG, widthE, widthH, widthA, angleAlfa, angleBeta)
        {
            ...
            return `
                <svg class="configurated"  
                    ...
                    <path d="
                        M ${SHAPE_START_X} 
                          ${left ? SHAPE_START_Y + widthH - widthA : SHAPE_START_Y}
                        L ${SHAPE_START_X + widthG * Math.cos(betaRadians)} 
                          ${left ? SHAPE_START_Y + widthH - widthA - widthG * Math.sin(betaRadians) : SHAPE_START_Y - widthG * Math.sin(betaRadians)}
                        L ${SHAPE_START_X + widthG * (1- Math.sin(betaRadians)) + widthA * Math.sin(betaRadians)} 
                          ${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference - widthG : SHAPE_START_Y + bigger * Math.cos(betaRadians) - widthG}
                        L ${SHAPE_START_X + widthE - widthG * (1 - Math.sin(alfaRadians)) + widthA * Math.sin(betaRadians)} 
                          ${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference - widthG : SHAPE_START_Y + bigger * Math.cos(betaRadians) - widthG}
                        L ${SHAPE_START_X + widthE - widthG * Math.cos(alfaRadians) + widthA * Math.sin(betaRadians) + widthH * Math.sin(alfaRadians)} 
                          ${left ? SHAPE_START_Y - widthA + widthA * Math.cos(betaRadians ) + widthH * (1-Math.cos(alfaRadians)) - widthG * Math.sin(alfaRadians) : SHAPE_START_Y - widthH + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians))}
                        L ${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians) + widthH * Math.sin(alfaRadians)} 
                          ${left ? SHAPE_START_Y - widthA + widthA * Math.cos(betaRadians ) + widthH * (1-Math.cos(alfaRadians)) : SHAPE_START_Y - widthH + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians))}
                        L ${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)} 
                          ${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference : SHAPE_START_Y + bigger * Math.cos(betaRadians)}
                        L ${SHAPE_START_X + widthA * Math.sin(betaRadians)} 
                          ${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference : SHAPE_START_Y + bigger * Math.cos(betaRadians)}
                        Z
                    " fill="var(--color-fill)" stroke="black" stroke-width="${BIG_STROKE_WIDTH}"/>
<!--                    E - line -->
                    <line x1="${SHAPE_START_X + widthA * Math.sin(betaRadians)}" 
                          x2="${SHAPE_START_X + widthA * Math.sin(betaRadians)}" 
                          y1="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference : SHAPE_START_Y + bigger * Math.cos(betaRadians)}" 
                          y2="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference + MEASURING_LINE_LENGTH : SHAPE_START_Y + bigger * Math.cos(betaRadians) + MEASURING_LINE_LENGTH}" 
                          stroke="gray" 
                          stroke-width="${SMALL_STROKE_WIDTH}"/>
                    <line x1="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)}" 
                          x2="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)}" 
                          y1="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference : SHAPE_START_Y + bigger * Math.cos(betaRadians)}" 
                          y2="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference + MEASURING_LINE_LENGTH : SHAPE_START_Y + bigger * Math.cos(betaRadians) + MEASURING_LINE_LENGTH}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
<!--                    E - txt -->
                    <line x1="${SHAPE_START_X + widthA * Math.sin(betaRadians)}" 
                          x2="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians)}" 
                          y1="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference + MEASUREMENT_MARGIN : SHAPE_START_Y + bigger * Math.cos(betaRadians) + MEASUREMENT_MARGIN}" 
                          y2="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference + MEASUREMENT_MARGIN : SHAPE_START_Y + bigger * Math.cos(betaRadians) + MEASUREMENT_MARGIN}" stroke="gray" stroke-width="${BIG_STROKE_WIDTH}" marker-start="url(#arrowhead)" marker-end="url(#arrowhead)"/>
                    <text x="${SHAPE_START_X + widthA * Math.sin(betaRadians) + widthE*0.5 - 2}" 
                          y="${left ? SHAPE_START_Y + smaller * Math.cos(betaRadians) + difference + MEASUREMENT_MARGIN - 2.5 : SHAPE_START_Y + bigger * Math.cos(betaRadians) + MEASUREMENT_MARGIN - 2.5}" 
                          font-size="10" 
                          font-family="sans-serif" 
                          fill="gray">E</text>

<!--                    H - line-->
                    <line x1="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians) + widthH * Math.sin(alfaRadians)}" 
                          x2="${SHAPE_START_X + widthE + MEASURING_LINE_LENGTH * Math.cos(alfaRadians) + widthA * Math.sin(betaRadians) + widthH * Math.sin(alfaRadians) }" 
                          y1="${left ? SHAPE_START_Y - widthA + widthA * Math.cos(betaRadians ) + widthH * (1-Math.cos(alfaRadians)) : SHAPE_START_Y - widthH + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians))}" y2="${left ? SHAPE_START_Y - widthA + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians)) + MEASURING_LINE_LENGTH * Math.sin(alfaRadians): SHAPE_START_Y - widthH + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians)) + MEASURING_LINE_LENGTH * Math.sin(alfaRadians)}" stroke="gray" stroke-width="${SMALL_STROKE_WIDTH}"/>
...
<!--                    H - txt -->
                    ...
                    <text x="${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians) + widthH * Math.sin(alfaRadians) - 4.5 + widthH * 0.5}" y="${left ? SHAPE_START_Y - widthA + widthA * Math.cos(betaRadians ) + widthH * (1-Math.cos(alfaRadians)) - MEASUREMENT_MARGIN - 2.5 : SHAPE_START_Y - widthH + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians)) - MEASUREMENT_MARGIN - 2.5}" font-size="10" font-family="sans-serif" transform="rotate(${angleAlfa} ${SHAPE_START_X + widthE + widthA * Math.sin(betaRadians) + widthH * Math.sin(alfaRadians)} ${left ? SHAPE_START_Y - widthA + widthA * Math.cos(betaRadians ) + widthH * (1-Math.cos(alfaRadians)) : SHAPE_START_Y - widthH + widthA * Math.cos(betaRadians) + widthH * (1-Math.cos(alfaRadians))})" fill="gray">H</text>
...
<!--                    A - txt -->
                    ...
                    <text x="${SHAPE_START_X - 2.5 - widthA * 0.5}" y="${left ? SHAPE_START_Y + widthH - widthA - MEASUREMENT_MARGIN - 2.5 : SHAPE_START_Y - MEASUREMENT_MARGIN - 2.5}" font-size="10" font-family="sans-serif" transform="rotate(${-angleBeta} ${SHAPE_START_X} ${left ? SHAPE_START_Y + widthH - widthA : SHAPE_START_Y})" fill="gray">A</text>
...
<!--                    G - txt -->
                    ...
                    <text x="${SHAPE_START_X + positionGLabel + widthA * Math.sin(betaRadians) + 0.5 * MEASURING_LINE_LENGTH}" y="${left ? SHAPE_START_Y+smaller*Math.cos(betaRadians)-widthG + difference : SHAPE_START_Y+bigger*Math.cos(betaRadians)-widthG}" font-size="10" font-family="sans-serif" transform="rotate(-90 ${SHAPE_START_X + positionGLabel + widthA * Math.sin(betaRadians)} ${left ? SHAPE_START_Y+smaller*Math.cos(betaRadians)-widthG + difference : SHAPE_START_Y+bigger*Math.cos(betaRadians)-widthG})" fill="gray">g</text>
                </svg>
            `
        },
